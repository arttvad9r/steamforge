...
data class ContractCounters(
    /** Total score accumulated during the contract day. */
    val score: Int = 0,
    ...
    fun record(event: GameEvent): ContractCounters = when (event) {
        ...
        is GameEvent.TileCreated -> {
            val level = event.level.coerceIn(1, 30)
            val count = event.count.coerceAtLeast(0)
            if (count == 0) this else copy(
                madeTilesByLevel = madeTilesByLevel +
                    (level to saturatingAdd(madeTileCount(level), count)),
            )
        }
